...
- README / docs 与当前代码同步：内联事件桥接、架构与错误码手册更新；
- 清理过期规划文档与 adapters 相关历史表述。
...
- 删除 `adapters/*` 子模块（authx/cachex/clix/dbx/filex/jobx，家族生产零引用）；各库事件桥由使用方内联实现（示例已改为内联桥）；
- CI/Release 工作流清理 adapters 残留。
...
- 依赖升级：errx v1.5.7、logx v1.3.4、testx v1.4.5、validx v1.2.5（go get -u -t all）。
